...
template <>
void OctomapGenerator<PCLSemanticsJaguar, SemanticsOcTreeJaguar>::updateColorAndSemantics(PCLSemanticsJaguar *pcl_cloud)
{
  for (PCLSemanticsJaguar::const_iterator it = pcl_cloud->begin(); it < pcl_cloud->end(); it++)
  {
    if (!std::isnan(it->x) && !std::isnan(it->y) && !std::isnan(it->z))
    {
      // Get semantics
...
template class OctomapGenerator<PCLSemanticsJaguar, SemanticsOcTreeJaguar>;
