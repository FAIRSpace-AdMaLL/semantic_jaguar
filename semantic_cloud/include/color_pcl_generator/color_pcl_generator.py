...
if __name__ == "__main__":
    ...
    cloud_gen = ColorPclGenerator(intrinsic, color_img.shape[1], color_img.shape[0], frame_id="map")

    print("intrinsic matrix", intrinsic)
    # Generate point cloud and pulish ros message
    while not rospy.is_shutdown():
        since = time.time()
        cloud_ros = cloud_gen.generate_cloud_color(color_img, depth_img, rospy.Time.now()) #intrinsic)
